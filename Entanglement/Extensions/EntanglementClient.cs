...
namespace Ace.Networking.Entanglement.Extensions
{
    ...
    {
        public static async Task<T> Entangle<T>(this IConnection connection, Guid? eid = null) where T : class, IEntangledObject
        {
            var interfaceId = typeof(T).GetTypeInfo().GUID;
            T instance = getExistingInstance<T>(connection, eid);
        ...
        private static T getExistingInstance<T>(this IConnection connection, Guid? eid) where T : class, IEntangledObject
        {
            if (eid.HasValue)
            {
